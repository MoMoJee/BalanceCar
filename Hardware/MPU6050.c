...
/**
  ...
  * 返 回 值：无
  */
static void MPU6050_ReadBytes(uint8_t RegAddress, uint8_t *Buffer, uint8_t Count)
{
	uint8_t i;
	...
	MPU6050_WaitEvent(I2C2, I2C_EVENT_MASTER_BYTE_RECEIVED);
	Buffer[Count - 1] = I2C_ReceiveData(I2C2);
	I2C_AcknowledgeConfig(I2C2, ENABLE);
}
...
void MPU6050_Init(void)
{
	...
	MPU6050_WriteReg(MPU6050_SMPLRT_DIV, 0x01);				//采样率分频寄存器，提高数据更新频率
	MPU6050_WriteReg(MPU6050_CONFIG, 0x02);					//配置寄存器，提高带宽，降低滤波延迟
...
{
	uint8_t Data[14];								//定义数据缓存区

	MPU6050_ReadBytes(MPU6050_ACCEL_XOUT_H, Data, 14);	//读取加速度计和陀螺仪数据
