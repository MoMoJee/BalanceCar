...
/*--------------main----------------*/
void OLED_screentest(void)
{
	OLED_ShowImage(0, 0, 16,16, test);	
	OLED_ShowImage(0, 16, 16,16, test);
	OLED_ShowImage(16, 0, 16,16, test);
	OLED_ShowImage(16, 16, 16,16, test);
	OLED_DrawRectangle(32, 0, 96, 32, 0);
	OLED_DrawEllipse(80, 16, 47, 15, 0);
	OLED_ShowImage(0, 48, 16,16, test);	
	OLED_ShowImage(0, 32, 16,16, test);
	OLED_ShowImage(16, 32, 16,16, test);
	OLED_ShowImage(16, 48, 16,16, test);	
	OLED_ShowImage(96, 48, 16,16, test);	
	OLED_ShowImage(96, 32, 16,16, test);
	OLED_ShowImage(112, 32, 16,16, test);
	OLED_ShowImage(112, 48, 16,16, test);
	OLED_ShowImage(32, 32, 64,16, test1);
	OLED_ShowImage(32, 48, 64,16, test2);
}

void OLED_smartcarsystem(void)
{
	OLED_smartcarsystem1();
	OLED_Clear();	
	OLED_screentest();	
	OLED_Update();
	OLED_Clear();
	Delay_s(1);
	OLED_smartcarsystem2();	
	OLED_Update();
	OLED_Clear();
}

void OLED_smartcarsystem1(void)
{
	OLED_ShowChinese(8, 0, "智能车调试系统");
	OLED_ShowImage(0, 16, 128,40, MUC);
	int i=0;
	int a=0;
	for(i=0;i<23;i++)
	{
		OLED_ShowString(a, 56, "#",6);
		a+=6;
		OLED_Update();
		Delay_ms(30);//100
	}
...
void OLED_smartcarsystem2(void)
{	
	OLED_ShowImage(0, 0, 16,16, BLE_off);
	OLED_ShowImage(16, 0, 16,16, HC_off);
	OLED_ShowImage(112, 0, 16,16, Light_off);
	OLED_ShowString(0, 16, "waiting signal...",6);
	OLED_ShowString(0, 24, "waiting commands...",6);
}
